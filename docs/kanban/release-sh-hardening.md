...
- 2026-06-16: Done. Implemented in 4 commits on master: cliff.toml v-prefix fix + CHANGELOG backfill (b1d317b), pre-flight checks (branch/clean/sync/tag-exists), ERR trap rollback, annotated tag + interactive push prompt. Pre-flight smoke-tested: dirty-tree guard fired correctly on first manual run. Released as v0.3.3 (next release will use the hardened script).
